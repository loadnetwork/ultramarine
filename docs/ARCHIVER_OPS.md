...
```toml
[archiver]
# Enable/disable the archiver worker
enabled = true
...
- **No uploads**: Blobs are never uploaded to external storage
- **No pruning**: Blobs remain in local storage indefinitely
- **Full serving**: All blob requests are served from local storage
- **Consensus retention unaffected**: The node skips tracking `pending_archive_heights`, so the regular consensus store pruning window continues to apply even though blob bytes accumulate.

This is useful for:
- Archive nodes that want to retain all historical blobs
- Testing/development environments
- Nodes without external storage access
