...
When the archiver is disabled (`archiver.enabled = false`), `pending_archive_heights` is never populated, so the consensus store’s own pruning window is unaffected while blob bytes remain on disk for manual/long-term retention.
...
- `crates/test/tests/full_node/node_harness.rs`:
  ...
  - archiver disabled: no pruning

🟡 Remaining to add/expand (for Phase 6 sign-off)
- Multi-node (2f+1) scenario where proposer uploads, followers verify notices, and all prune after finality.
...
- ⚠️ “No retention window” applies to *blob bytes*. The consensus store still has its own pruning (`Store::prune()` in `State::commit`) that controls how much decided history is kept. These are separate mechanisms and should be configured consciously.
- ⚠️ If archiver is disabled permanently, blobs stay local and no archive/prune automation runs. The consensus store now skips `pending_archive_heights` in this mode, so normal decision-history pruning applies while blob bytes accumulate on disk.
