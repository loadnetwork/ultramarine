...
impl ExecutionClient {
    ...
    pub async fn new(config: ExecutionConfig) -> eyre::Result<Self> {
        ...
        let forkchoice_with_attrs_max_attempts = config
            .forkchoice_with_attrs_max_attempts
            .unwrap_or(10)
            .max(1);
        let forkchoice_with_attrs_delay = Duration::from_millis(
            config.forkchoice_with_attrs_delay_ms.unwrap_or(200),
        );
    ...
    async fn forkchoice_updated_with_attributes_retry(
        ...
        loop {
            ...
            match res.payload_status.status {
                ...
                PayloadStatusEnum::Syncing => {
                    // Engine API spec: SYNCING commonly returns latestValidHash=null and payloadId=null.
                    // See `execution-apis/src/engine/paris.md` (forkchoiceUpdated spec point 8).
                ...
                PayloadStatusEnum::Accepted => {
                    // `ACCEPTED` is not listed as a valid response for forkchoiceUpdated in the spec.
                    // Be liberal:
                    // - If the EL provides a payloadId, proceed so proposers can keep producing blocks.
...
mod tests {
    ...
    use alloy_primitives::{
        Address as AlloyAddress, Bloom, Bytes as AlloyBytes, FixedBytes, B256 as AB256, U256,
    };
    ...
    impl EngineApi for ScriptedEngineApi {
        ...
        async fn get_payload(&self, _payload_id: PayloadId) -> eyre::Result<ExecutionPayloadResult> {
            self.payloads
                .lock()
                .unwrap()
                .last()
    ...
    fn sample_payload_result(block_hash: AB256, parent_hash: AB256, block_number: u64) -> ExecutionPayloadResult {
    ...
    fn make_latest_block(block_hash: AB256, number: u64, parent_hash: AB256, timestamp: u64) -> ultramarine_types::engine_api::ExecutionBlock {
    ...
    async fn accepted_with_payload_id_proceeds_to_get_payload() {
        ...
        assert_eq!(
            res.payload.payload_inner.payload_inner.parent_hash,
            head,
