        ...
        sync::Response::VoteSetResponse(vote_set_response) => proto::SyncResponse {
            response: Some(proto::sync_response::Response::VoteSetResponse(
                proto::VoteSetResponse {
                    ...
                    round: vote_set_response.round.as_u32().expect("round should not be nil"),
...
pub fn encode_certificate(
    ...
    Ok(proto::CommitCertificate {
        ...
        aggregated_signature: Some(encode_aggregate_signature(&certificate.aggregated_signature)?),
    })
}

pub fn decode_aggregated_signature(
    signature: proto::AggregatedSignature,
) -> Result<AggregatedSignature<LoadContext>, ProtoError> {
...
pub fn encode_aggregate_signature(
    aggregated_signature: &AggregatedSignature<LoadContext>,
...
pub fn encode_vote_set(vote_set: &VoteSet<LoadContext>) -> Result<proto::VoteSet, ProtoError> {
    Ok(proto::VoteSet {
        signed_votes: vote_set.votes.iter().map(encode_vote).collect::<Result<Vec<_>, _>>()?,
    })
}
...
pub fn decode_vote_set(vote_set: proto::VoteSet) -> Result<VoteSet<LoadContext>, ProtoError> {
    Ok(VoteSet { votes: vote_set.signed_votes.into_iter().filter_map(decode_vote).collect() })
}
