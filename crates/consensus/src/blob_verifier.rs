...
impl BlobVerifier {
    ...
    pub fn new() -> Result<Self, BlobVerificationError> {
        ...
        let trusted_setup: TrustedSetup = serde_json::from_slice(TRUSTED_SETUP_BYTES)
            .map_err(|e| BlobVerificationError::TrustedSetupLoad(format!("Failed to parse trusted setup: {:?}", e)))?;
        ...
        .map_err(|e| BlobVerificationError::TrustedSetupLoad(format!("Failed to load KZG settings: {:?}", e)))?;
    ...
    pub fn from_trusted_setup_file(
        ...
        let bytes = std::fs::read(trusted_setup_path)
            .map_err(|e| BlobVerificationError::TrustedSetupLoad(format!("Failed to read file: {:?}", e)))?;
