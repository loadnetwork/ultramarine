...
use ultramarine_execution::error::ExecutionError;
    ...
    E: BlobEngine,
{
    ...
    pub async fn prepare_blob_sidecar_parts(
        ...
        let header_message = self
            .build_sidecar_header_message(value.height, &self.address, metadata, body_root, parent_root)
        ...
        Notify: ExecutionNotifier,
    {
        ...
        let executed_is_parent =
            self.executed_height.as_u64().saturating_add(1) == height.as_u64();
            ...
            loop {
                match notifier.set_latest_forkchoice_state(block_hash).await {
                    ...
                    Err(e) => {
                        ...
                        if looks_transient && start.elapsed() <= self.execution_retry.forkchoice_timeout {
                            warn!(
                                height = %height,
                                ?block_hash,
                                %msg,
                        ...
                        if looks_transient {
                            self.mark_el_degraded(format!(
                                ...
                                self.execution_retry.forkchoice_timeout,
                                height,
                                block_hash
                            ));
                            execution_pending = true;
